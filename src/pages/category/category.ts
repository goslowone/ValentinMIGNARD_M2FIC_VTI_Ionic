...
import { SingleItemClimber } from '../../pages/single-item-climber/single-item-climber';
...
export class CategoryPage {
  ...
  ionViewDidLoad() {
    ...
    let loader = this.loadingController.create({
      content: "Chargement de la liste des grimpeurs.."
    });
    loader.present();
  ...
  itemTapped($event, item) {
    if (item.category=='Spot outdoor' || item.category=='Spot indoor'){
      this.navCtrl.push(SingleItem, item);
    }
    else 
      this.navCtrl.push(SingleItemClimber, item);
